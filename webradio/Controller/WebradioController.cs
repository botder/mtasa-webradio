...
namespace Webradio.Controller
{
    ...
    public class WebradioController : ControllerBase
    {
        ...
        [NonAction]
        private ActionResult SearchFailure(string error)
        {
        ...
        [NonAction]
        private string GenerateCacheKey(string action, string service, string identifier)
        {
            using var sha256 = SHA256.Create();
            byte[] hash = sha256.ComputeHash(Encoding.UTF8.GetBytes($"{action}-{service}-{identifier.Trim().ToLower()}"));
        ...
        private void SetCacheString(string key, string value, long expirationOffsetInSeconds)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    logger.LogError($"SetCacheString: Cache took too long to respond (> 3 seconds) [key: {key}]");
                }
                catch (Exception cacheException)
                {
                    logger.LogError($"SetCacheString: Cache threw an exception: {cacheException}");
                }
            });
        }
        ...
        private void RemoveCacheString(string key)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    logger.LogError($"RemoveCacheString: Cache threw an exception: {cacheException}");
                }
            });
        }
    }
